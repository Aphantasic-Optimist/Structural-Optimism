---
title: Statistical evidence (OR = 1.50, 95% CI 1.42-1.59)
description: Statistical evidence (OR = 1.50, 95% CI 1.42-1.59)
---

# Statistical evidence (OR = 1.50, 95% CI 1.42-1.59)

**Type:** Entity  
**Confidence:** ★★★★★ Established

---

## Summary

Meta-analysis by Holt-Lunstad et al. (2010) of 148 studies with 308,849 participants found social connection reduces mortality risk by 50% (OR = 1.50, 95% CI 1.42-1.59). Effect comparable to smoking cessation, exceeds obesity/inactivity. Replicated across populations.

---

## Timeline

**Added to Knowledge Graph:** 2026-01-27 14:28 UTC  
**Valid Since:** 2010-01-01  

---

## Additional Information

**Source:** Holt-Lunstad et al. 2010  
**Year:** 2010  
**Finding:** Social connection reduces mortality risk by 50% (OR = 1.50, 95% CI 1.42-1.59)  
**Methodology:** Meta-analysis of 148 studies with 308,849 participants  
**Quality:** 5  
**Replication Status:** REPLICATED  

---

## Relationships

### Supported By

- **FOUND_EVIDENCE_FOR**: [Meta-analysis of 148 studies with 308,849 participants](../evidence/meta-analysis-of-148-studies-with-308849-participants.md)  
  *Meta-analysis found odds ratio 1.50 with 95% confidence interval 1.42-1.59 as evidence*
  📅 Valid since: 2010-01-01
  📚 Mentioned in 1 episode(s)


### Supports

- **SUPPORTS**: [Social connection reduces mortality risk by 50%](../claims/02-social-connection-reduces-mortality.md)  
  *Statistical evidence with odds ratio of 1.50 and 95% confidence interval 1.42-1.59 supports the claim that social connection reduces mortality risk by 50%*
  📚 Mentioned in 3 episode(s)


---

## Explore Further

[View in Knowledge Graph](../graph.md) to see all connections and explore related concepts.

---

*This page was auto-generated from the knowledge graph.*  
*Last updated: 2026-01-27*
