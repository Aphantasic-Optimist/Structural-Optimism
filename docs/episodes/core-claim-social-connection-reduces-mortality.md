---
title: Core Claim: Social Connection Reduces Mortality
description: Meta-analysis synthesis
---

# Core Claim: Social Connection Reduces Mortality

**Type:** Text  
**Published:** 2026-02-04  
**Added to Graph:** 2026-02-04  
**Source:** Meta-analysis synthesis

---

## Content

Social connection reduces all-cause mortality by approximately 50%. This is based on a meta-analysis of 148 studies with over 300,000 participants (Holt-Lunstad et al. 2010). The effect size is comparable to quitting smoking.

CONFIDENCE: ★★★★★ (Established)
SOURCES: Holt-Lunstad et al. 2010, PLoS Medicine

---

## Knowledge Graph Impact

This episode created or referenced **5 relationship(s)** in the knowledge graph:

- [Social connection](../entities/social-connection.md) **REDUCES** [All-cause mortality](../entities/all-cause-mortality.md)
  *Social connection reduces all-cause mortality by approximately 50%*

- [Social connection](../entities/social-connection.md) **BASED_ON** [Meta-analysis](../evidence/meta-analysis.md)
  *The finding about social connection reducing mortality is based on a meta-analysis*

- [Effect size](../entities/effect-size.md) **COMPARABLE_TO** [Smoking cessation](../entities/smoking-cessation.md)
  *The effect size of social connection on mortality is comparable to quitting smoking*

- [Meta-analysis](../evidence/meta-analysis.md) **CONDUCTED_BY** [Holt-Lunstad et al. 2010](../researchers/holt-lunstad-et-al-2010.md)
  *The meta-analysis was conducted by Holt-Lunstad et al. 2010*

- [Holt-Lunstad et al. 2010](../researchers/holt-lunstad-et-al-2010.md) **PUBLISHED_IN** [PLoS Medicine](../entities/plos-medicine.md)
  *Holt-Lunstad et al. 2010 study was published in PLoS Medicine*


---

## Explore Further

[View in Knowledge Graph](../graph.md) to see how this episode connects to other research.

---

*This page was auto-generated from the knowledge graph.*  
*Episode added: 2026-02-04*
